<?php
	...
	*/
	...
	/**
	*
	*	Vezérlő: admin_articleType.php
	*	Az oldal cikk tipusai
	*
	*/
	class admin_articleType extends helperClass
	{
		...
		# a lapozó URL cime
		var $pageTurnerURL = 'admin_articleType/show/';
		
		# listázáskor az oldal feléc információja
		var $pageHeader_show = 'Cikk tipusok';
		
		# új elem hozzáadásakor az oldal fejléc információja
		var $pageHeader_addNew = 'Új cikk hozzáadása';
		
		# adatok szerkesztésekor az oldal fejléc infrmációja
		var $pageHeader_edit = 'Cikk szerkesztése';
		
		# tábla mezők neve
		var $fieldName = array('ID', 'Title', 'Módositás');
		
		# a szerkesztése nevű formhoz az ACTION=URL
		var $formAction_edit = 'admin_articleType/edit/';
		
		# a hozzáadás nevű formhoz az ACTION=URL
		var $formAction_addNew = 'admin_articleType/addNew/';
		
		# a törléséhez az URL
		var $Delete = 'admin_articleType/Delete/';
		
		# form mezők nevei asszociativ tömben
		var $formFieldsName = array('title' => 'Title', 
									'content' => 'Content',
									'page' => 'Page',
									'img' => 'Image'
									);
		...
		var $sql_select = 'id, title';
		
		# a kép elérési útja, mappa/folder név
		var $imgUpLoad_DIR_name = 'images/';
		
		# a template-ben be/ki kapcsolhatjuk az export linket
		...
		*/
		function show()
		{
			# template ellemek ki/be kapcsolása
			...
			# table_name, id, sql_select, listLimit, linklimit, pageTurnerURL --- 6 db paramétert vár a függvény ---
		...
		*/
		function edit()
		{
			...
			foreach($listing as $key => $item)
			{
				...
				$form->textarea_tiny($this->formFieldsName['content'], 'content', 20, 80, 'id_content', $item['content']);
				$form->input($this->formFieldsName['img'], 'file', 'src_img', $item['src_img'], 'id_img', $this->imgUpLoad_DIR_name, 0);
				$form->input('', 'submit', 'edit_button', 'Módsitás', 'id_EditButton');
			...
			if($form->registFormCheck())
			{
				...
				elseif(isset($_POST['holder']))
				{	
					$file_name = $_POST['holder'];
				}
					
				$sql = "UPDATE $this->DB_tableName SET title = '$_POST[title]', content = '$_POST[content]', 
						src_img = '$file_name' WHERE id = '$_GET[id]'";
						# page = '$_POST[page]'
		...
		*/
		function addNew()
		{
			...
			$form = new formFactory;
			# a form generálásnál szigorú a sorrend, mivel van egy form ellem számláló ami kötötté teszi a sorrendet
			# ezért muszaj az ellenőrzés előtt legyen a form factory
			# formFactory.php - count_form_element
			...
			#$form->input('Teszt', 'text', 'test', '', 'id_test', 'equal', 1);
			$form->textarea_tiny($this->formFieldsName['content'], 'content', 20, 80, 'id_content', $var_content);
			# <input type=\"$type\" name=\"$name\" id=\"$id\" value=\"$value\" />
			# $form->input('Datum', 'date', 'date', 'id_date', '');
			
			$form->input($this->formFieldsName['img'], 'file', 'src_img', '', 'id_img');
			$form->input('', 'submit', 'add_button', 'Hozzáad', 'id_addButton');
			$form->html_form .= $form->form_end;
			...
			if($form->registFormCheck())
			{
				...
				$file_name = $img->upload($this->imgUpLoad_DIR_name);
				# SQL
				$sql_insert = "INSERT INTO $this->DB_tableName (title, content, src_img) 
							VALUES ('$_POST[title]', '$_POST[content]', '$file_name')";
